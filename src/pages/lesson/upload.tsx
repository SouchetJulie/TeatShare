...
const upload: FunctionComponent = () => {
  ...
  const onSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    if (success) {
      dispatch(addAlert({message: `Lesson ${id} créée avec succès !`, success}));
    }
  }
  ...
  return (
    <>
      <Container className="min-vh-100">
        <Button onClick={() => dispatch(addAlert({message: "test", success: Math.random() > 0.5}))}>Test</Button>
        <Row>
          <h1 className="mt-4">Importer un PDF</h1>
        </Row>
        <Form className="my-5 row" onSubmit={onSubmit}>
